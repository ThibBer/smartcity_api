...
module.exports.filter = async (client, filter) => {
    if(filter !== undefined){
        return await client.query("SELECT r.*, row_to_json(rt) as report_type, row_to_json(u) as reporter FROM report r LEFT JOIN reportType rt ON r.report_type = rt.id LEFT JOIN \"User\" u on r.reporter = u.id WHERE r.id::varchar(11) ~ $1 OR r.description ~ $1 OR r.state ~ $1 OR r.city ~ $1 OR r.street ~ $1 OR r.zip_code::varchar(11) ~ $1 OR r.house_number::varchar(11) ~ $1 OR r.reporter::varchar(11) ~ $1 OR r.report_type::varchar(11) ~ $1 ORDER BY r.created_at DESC, r.id", [filter])
    }

    return await client.query("SELECT r.*, row_to_json(rt) as report_type, row_to_json(u) as reporter FROM report r LEFT JOIN reportType rt ON r.report_type = rt.id LEFT JOIN \"User\" u on u.id = r.reporter ORDER BY r.created_at DESC, r.id", []);
}
...
module.exports.filterWithOffsetLimit = async (client, filter, offset, limit) => {
    ...
    return await client.query("SELECT r.*, row_to_json(rt) as report_type, row_to_json(u) as reporter FROM report r LEFT JOIN reportType rt ON r.report_type = rt.id LEFT JOIN \"User\" u on u.id = r.reporter ORDER BY r.created_at DESC, r.id OFFSET $1 LIMIT $2", [offset, limit]);
}
...
module.exports.getWithUserId = async (client, reporterId) => {
    return await client.query("SELECT r.*, row_to_json(rt) as report_type, row_to_json(u) as reporter FROM report r LEFT JOIN reportType rt ON r.report_type = rt.id LEFT JOIN \"User\" u on u.id = r.reporter WHERE r.reporter = $1 ORDER BY r.created_at DESC", [reporterId]);
}
